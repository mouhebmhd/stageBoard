...
async function loginAsInternService(email, password, role) {
    ...
    try {
        ...
        const searchQuery = `SELECT * FROM INTERNS WHERE internEmail ='${email}'`;
            ...
            connection.query(searchQuery, async (error, result, fields) => {
                if (error) {
                    ...
                    if (result.length === 0) {
                        ...
                        if (passwordMatch) {
                            resolve({status:"success",message:"account  found  ",userId:user.internId,userEmail:user.internEmail}); // Passwords match, return the user
                        } else {
